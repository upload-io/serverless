  ...
  getStackInfo() {
    ...
    return BbPromise.all(sdkRequests).then(() => {
      ...
      if (stackData.outputs) {
        ...
        // CloudFront
        const cloudFrontDomainNames = outputs.filter(output =>
          output.OutputKey.startsWith(
            this.provider.naming.getCloudFrontDistributionDomainNameLogicalId({})
          )
        );
        if (cloudFrontDomainNames.length > 0) {
          this.gatheredData.info.cloudFront = cloudFrontDomainNames.map(x => x.OutputValue);
        }
