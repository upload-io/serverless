...
class AwsCompileCloudFrontEvents {
  constructor(serverless, options) {
    ...
    const distributionObjectSchema = {
      type: 'object',
      additionalProperties: false,
      required: ['name'],
      properties: {
    ...
    this.serverless.configSchemaHandler.defineFunctionEvent('aws', 'cloudFront', {
      type: 'object',
      properties: {
        distribution: distributionObjectSchema,
  ...
  // Unfortunately Lodash doesn't seem to natively support a 'groupBy' where the key is an object.
  // Serializing the key to JSON isn't safe either -- it would require sorting all the object's keys, recursively.
  groupByObject(collection, keyName) {
    const groups = [];
    collection.forEach(item => {
      const groupKey = item[keyName];
      const existingGroup = groups.find(g => _.isEqual(g.key, groupKey));
      if (existingGroup) {
        existingGroup.values.push(item);
      } else {
        const newGroup = { key: groupKey, values: [item] };
  ...
  compileCloudFrontEvents() {
    const distributionsAndFunctionsJoined = _.flatten(
      this.serverless.service.getAllFunctions().map(functionName => {
        const functionObj = this.serverless.service.getFunction(functionName);
        return (functionObj.events || []).map(event => {
          const distribution = (event.cloudFront || {}).distribution || {};
          return { distribution, functionName };
        });
      })
    );

    const functionsGroupedByDistribution = this.groupByObject(
      distributionsAndFunctionsJoined,
      'distribution'
    ).map(({ key, values }) => ({
      distribution: key,
      functionNames: values.map(x => x.functionName),
    }));

    const distributionNames = functionsGroupedByDistribution
      .map(x => x.distribution.name)
      .filter(x => x);

    const sameNameDistributionsWithDifferentProperties = _.toPairs(_.groupBy(distributionNames))
      .filter(([, cardinality]) => cardinality.length > 1)
      .map(([distribution]) => distribution);

    if (sameNameDistributionsWithDifferentProperties.length > 0) {
      throw new ServerlessError(
        `The following cloudfront distribution names are used in multiple places, but with different properties (which is not allowed): ${sameNameDistributionsWithDifferentProperties.join(
          ', '
        )}`
      );
    }

    functionsGroupedByDistribution.forEach(({ distribution, functionNames }) => {
      const cloudFrontDistributionLogicalId = this.provider.naming.getCloudFrontDistributionLogicalId(
        distribution
      );

      const cloudFrontDistributionDomainNameLogicalId = this.provider.naming.getCloudFrontDistributionDomainNameLogicalId(
        distribution
      );
      ...
      function createOrigin(origin, naming) {
        ...
        if (typeof origin === 'string') {
          ...
          } else if (originUrl.protocol === 'group:') {
            const originGroupId = url.parse(origin, false, true).host;
            return {
              targetOriginId: originGroupId,
              origin: null, // No origin to create: reference is to an origin group defined in the 'distribution' field.
            };
          } else {
            Object.assign(originObj, {
              CustomOriginConfig: {
        ...
        return { targetOriginId: originObj.Id, origin: originObj };
      }
        ...
        if (functionObj.events) {
          functionObj.events.forEach(event => {
            if (event.cloudFront) {
              ...
              const originObj = createOrigin(event.cloudFront.origin, this.provider.naming);
              let { origin } = originObj;
              const { targetOriginId } = originObj;
              if (origin) {
              ...
              if (event.cloudFront.behavior) {
                if (
                  ...
                  event.cloudFront.behavior.CachePolicyId !== undefined
                ) {
              ...
              if (event.cloudFront.cachePolicy) {
                ...
                // Backward compatibility - Assigning default cache policy only if none of the ForwardedValues, MinTTL, MaxTTL and DefaultTTL deprecated fields are defined.
      ...
      if (lambdaAtEdgeFunctions.length) {
        ...
        const originsWithSamePath = _.flatten(
          _.toPairs(_.groupBy(behaviors, 'PathPattern'))
            .filter(([, values]) => values.length > 1)
            .map(([, values]) => values)
        ).map(x => x.TargetOriginId);
        const originsInOriginGroups = _.flatten(
          (distribution.originGroups || []).map(x => x.members)
        );
        const originsWithSamePathNotInGroups = _.difference(
          originsWithSamePath,
          originsInOriginGroups
        );

        if (originsWithSamePathNotInGroups.length > 0) {
          throw new ServerlessError(
            'Found more than one behavior with the same PathPattern. Only origins in the same origin group can share the same PathPattern.'
          );
        }
                ...
                SourceArn: {
                  'Fn::Join': [
                    ...
                    [
                      ...
                      { Ref: 'AWS::AccountId' },
                      ':distribution/',
                      { Ref: cloudFrontDistributionLogicalId },
                    ],
                  ],
                },
              },
        ...
        if (!Resources.IamRoleLambdaExecution) {
          this.serverless.cli.log(
            chalk.magenta(
              'Remember to add required lambda@edge permissions to your execution role.'
            )
          );
          this.serverless.cli.log(
            chalk.magenta(
        ...
        const originGroupsData = distribution.originGroups || [];
        const OriginGroups = {
          OriginGroups: {
            Quantity: originGroupsData.length,
            Items: originGroupsData.map(originGroup => ({
              Id: originGroup.id,
              FailoverCriteria: {
                StatusCodes: {
                  Quantity: originGroup.failoverCriteria.length,
                  Items: originGroup.failoverCriteria,
                },
              },
              Members: {
                Quantity: originGroup.members.length,
                Items: originGroup.members.map(originId => ({
                  OriginId: originId,
                })),
              },
            })),
        ...
        const CloudFrontDistribution = {
          ...
          Properties: {
            DistributionConfig: {
              ...
              ...(originGroupsData.length > 0 ? OriginGroups : {}),
        ...
        Object.assign(Resources, { [cloudFrontDistributionLogicalId]: CloudFrontDistribution });

        _.merge(Outputs, {
          [cloudFrontDistributionLogicalId]: {
            Description: 'CloudFront Distribution Id',
            Value: {
              Ref: cloudFrontDistributionLogicalId,
            },
          },
          [cloudFrontDistributionDomainNameLogicalId]: {
            Description: 'CloudFront Distribution Domain Name',
            Value: {
              'Fn::GetAtt': [cloudFrontDistributionLogicalId, 'DomainName'],
            },
          },
        });
      }
